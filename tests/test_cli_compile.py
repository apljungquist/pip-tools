...
def test_build_distribution(
    ...
    """
    Test that when one or more ``--build-distribution`` are given the expected packages are
    included.
    """
    ...
    with runner.isolated_filesystem(tmp_path) as tmp_pkg_path:
        ...
        out = runner.invoke(
            cli,
            base_cmd
            + [f"--build-distribution={d}" for d in distributions]
            + other_options,
        )
...
def test_all_build_distributions(
    ...
    """
    Test that ``--all-build-distributions`` is equivalent to specifying every
    ``--build-distribution``.
    """
    make_wheel(make_package("small-fake-d", version="0.4"), fake_dists),
...
def test_only_build_distributions(fake_dists, runner, tmp_path, monkeypatch):
    """
    Test that ``--only-build-distributions`` excludes dependencies other than build dependencies.
    """
    expected_deps = {
        "setuptools": Version("*"),
        "small-fake-a": Version("0.1"),
    ...
    with runner.isolated_filesystem(tmp_path) as tmp_pkg_path:
        ...
        actual_out = runner.invoke(
            cli,
            base_cmd + ["--all-build-distributions", "--only-build-distributions"],
        )
...
def test_all_build_distributions_fail_with_build_distribution(
    ...
    assert out.exit_code == 2
    exp = "--build-distribution has no effect when used with --all-build-distributions"
    assert exp in out.stderr
...
def test_all_build_distributions_fail_non_build_distribution_options(runner, option):
    """
    Test that passing ``--all-build-distributions`` and conflicting option fails.
    """
    out = runner.invoke(
        cli,
        [
    ...
    assert out.exit_code == 2
    exp = "--only-build-distributions cannot be used with any of --extra, --all-extras"
    assert exp in out.stderr
